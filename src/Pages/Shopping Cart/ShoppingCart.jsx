...
const ShoppingCart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setItems(responseData);
        // Calculate total payable amount when items change
        calculateCartPayable(responseData);
      } catch (error) {
        setError(error);
      } finally {
        setLoading(false);
  ...
  const handleDeleteCartItem = async (itemId) => {
    try {
      ...
      const remainingCartItems = items.filter((item) => item._id !== itemId);
      setItems(remainingCartItems);
      // Recalculate total payable amount after item deletion
      calculateCartPayable(remainingCartItems);
    } catch (error) {
      console.error(`Error deleting item with ID ${itemId}:`, error);
    }
  };
